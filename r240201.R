...
print(d)
print(e)
e # 출력이 되는 것은 아님. 그냥 데이터 확인차원에서 보여지는 중
f = 1:10
print(d[1])
print(e[1])
...
list_a = list(name = 'test', age = 20, phone = '01012345678')# 키 값이 name, age, phone
print(list_a)
print(list_a['name'])
b = c('test', 20, '01012345678') # 키 값이 1, 2, 3
print(b[1])
...
df = data.frame(
  ...
  age = c(20, 30, 40),
  phone = c('01012345678', '01094561598', '01098765432')
)
print(df)
...
y = 3
print(x + y)
x - y # 데이터가 어딘가에 저장된 것은 아님
x - y -> result # 데이터를 result에 저장. 저장이 되면 콘솔에 결과는 안나옴
print(x * y)
print(x / y)
print(x ^ y)
print(x %% y) # 나머지
...
# 비교연산자
# 두 개의 데이터를 비교하여 결과 값은 논리값(참/거짓)으로 출력
z = 5
print(x == y)
print(y != z)
print(x > y)
...
print(TRUE & FALSE)
# or
# 두 개의 논리값 중 하나만 참이어도 참을 출력
# 두 개의 논리값 모두 거짓인 경우에만 거짓을 출력
print(TRUE | TRUE)
print(TRUE | FALSE)
print(FALSE | FALSE)
...
# else문에서 조건식이 거짓인 경우 실행할 코드를 작성
if (x >5){
  print('x는 5보다 크다')
}else{
  print('x는 5보다 작거나 같다')
...
if (score >= 90){
  ...
  # score가 90미만 80이상인 경우
  print('B')
}else if (score >= 70){
  # score가 80미만 70이상인 경우
  print('C')
}else {
  # 70미만인 경우
  print('D')
...
# while문
# 초기 시작값을 지정하고 해당하는 값을 반복 실행할 때마다 증감시켜
...
for (i in array_a){
  #print(i)
  # i값이 2부터 9일때 1부터 9까지 반복을 하는 반복문 생성
  for (j in array_b){
    print(i * j)
  }
}
...
## 1부터 100까지의 누적합 중에 누적합이 2000이 넘어가는 부분의 숫자는 몇인가
## 합계는 몇인가
result = 0
for (i in 1:100){
  if (result >= 2000){
...
## 1부터 1씩 증가시킨 데이터를 누적합을 하는 도중 누적합이 50,000이 넘어가는
## 최초의 순간은 언제인가?
i = 1
result = 0
while (TRUE){
...
## 1부터 100까지 숫자 중 짝수의 누적합을 출력
## 합계라는 초기 값을 0 대입하여 생성
result = 0
